-- ============================================================
-- AML Platform — Detection Engine Seed Data
-- Derived from Python AML detection engine run on 364 synthetic
-- transactions across 108 accounts.
-- 6 findings (all risk scores computed via SHAP-style additive
-- scoring in detect_patterns.py, not hardcoded).
--
-- Findings:
--   [Round-Trip]  Nexus Capital Ltd        risk=99  ₹4,78,50,123   hops=3
--   [Round-Trip]  Sharma Real Estate       risk=99  ₹15,68,92,410  hops=3
--   [Layering]    Kapoor Diamonds Pvt Ltd  risk=83  ₹4,86,06,321   hops=6
--   [Layering]    Rajan Exports Ltd        risk=83  ₹5,04,07,615   hops=5
--   [Smurfing]    Meera Textiles Corp      risk=74  ₹30,63,122     hops=0
--   [Smurfing]    Ananya Trading Pvt Ltd   risk=67  ₹21,29,029     hops=0
-- ============================================================
...
BEGIN
  ...
  -- ── ENTITIES ──────────────────────────────────────────────────────────
  INSERT INTO public.entities (id, entity_code, name, entity_type, risk_score, jurisdiction, linked_accounts, linked_cases, kyc_status, flagged_at)
  VALUES
  ...
  -- ── CASES ─────────────────────────────────────────────────────────────
  INSERT INTO public.cases (id, case_ref, subject, account_id, pattern, risk_score, amount, jurisdiction, case_status, days_open, assigned_officer, entity_id)
  VALUES
  ...
  -- ── ALERTS ────────────────────────────────────────────────────────────
  INSERT INTO public.alerts (id, alert_id, account_id, account_holder, pattern, risk_score, amount, jurisdiction, detected_at, alert_status, hops, assigned_to, case_id, entity_id)
  VALUES
  ...
  INSERT INTO public.sar_reports (sar_id, case_ref, subject, account_id, pattern, risk_score, amount, generated_at, sar_status, officer, fiu_ref, case_id)
  VALUES
  ...
  -- ── TRANSACTIONS (key flagged transactions from detection engine) ──────
  -- Round-Trip: Nexus Capital Ltd (INDUS-2850) — 3 hops via Cayman Islands
  INSERT INTO public.transactions (txn_id, from_account, to_account, amount_inr, amount_display, txn_type, jurisdiction, risk_score, flagged, hops, alert_id, case_id, txn_timestamp)
  VALUES
  ...
  -- Round-Trip: Sharma Real Estate (KOTAK-3010) — 3 hops via Antwerp
  INSERT INTO public.transactions (txn_id, from_account, to_account, amount_inr, amount_display, txn_type, jurisdiction, risk_score, flagged, hops, alert_id, case_id, txn_timestamp)
  VALUES
  ...
  -- Layering: Kapoor Diamonds Pvt Ltd (HDFC-1248) — 6 hops via Antwerp
  INSERT INTO public.transactions (txn_id, from_account, to_account, amount_inr, amount_display, txn_type, jurisdiction, risk_score, flagged, hops, alert_id, case_id, txn_timestamp)
  VALUES
  ...
  -- Layering: Rajan Exports Ltd (AXIS-8506) — 5 hops via Mauritius
  INSERT INTO public.transactions (txn_id, from_account, to_account, amount_inr, amount_display, txn_type, jurisdiction, risk_score, flagged, hops, alert_id, case_id, txn_timestamp)
  VALUES
  ...
  -- Smurfing: Meera Textiles Corp (BOB-2469) — 16 sub-threshold deposits
  INSERT INTO public.transactions (txn_id, from_account, to_account, amount_inr, amount_display, txn_type, jurisdiction, risk_score, flagged, hops, alert_id, case_id, txn_timestamp)
  VALUES
  ...
  -- Smurfing: Ananya Trading Pvt Ltd (CANARA-5685) — 11 sub-threshold deposits
  INSERT INTO public.transactions (txn_id, from_account, to_account, amount_inr, amount_display, txn_type, jurisdiction, risk_score, flagged, hops, alert_id, case_id, txn_timestamp)
  VALUES
  ...
  INSERT INTO public.dashboard_metrics (metric_key, metric_value, metric_label)
  VALUES
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'AML engine seed failed: %', SQLERRM;
END $$;
